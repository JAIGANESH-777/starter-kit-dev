...
- **Infra**: Port setup and Docker compose.
...
→ frontend-design + state-ssr (UI/state) → security-hardening (auth/CORS/cookies)
→ tdd (write tests for each vertical slice as you build it, not after). Only open 
...
5. Build a distinct UI for every auth method listed in Section 5 — don't default everything to 
...
6. Never hardcode secrets — only .env.example. Never modify the root docker-compose.yml directly; 
...
hydration warnings, the database is reachable through the configured ORM, and `docker-compose up` brings up the full 
